...
from model.dino_vision_transformer import vit_small as dino_vit_small
from model.mocov3_vits import vit_small as mocov3_vit_small
from model.resnet import resnet50
from config import PATH, CUB_PATH, IMAGENET_PATH
...
def main():
    ...
    parser.add_argument('--shot', default=1, type=int, choices=[1, 2, 3, 4, 5, 8, 16])
    ...
    mset = FewShotMetaSet(data_path=data_path, setname=args.dataset, shot=args.shot)
    ...
    if args.pretrain_method == 'MSN':
        ...
        if args.arch == 'deit_small_p16':
            model = dino_vit_small(patch_size=16)
        else:
            raise NotImplementedError
    elif args.pretrain_method == 'MoCov3':
        if args.arch == 'deit_small_p16':
            ...
            raise NotImplementedError
    elif args.pretrain_method in ['BYOL', 'SimCLR']:
    ...
    for rid, train_plain in enumerate(trains_plain):
        ...
        save_path_all = os.path.join(args.exp_dir, 'train_{}shot_{}.pth'.format(args.shot, rid))
