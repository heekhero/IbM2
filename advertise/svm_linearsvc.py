...
def main():
    ...
    parser.add_argument('--loss', default='hinge', choices=['hinge', 'squared_hinge'])
    parser.add_argument('--fit_intercept', default=False, action='store_true')

    parser.add_argument('--aug_100times', default=False, action='store_true')
    ...
    args.exp_dir_ft = os.path.join(args.exp_dir, 'CompareWithLinearSVM', 'aug100times_svm_linear_svc' if args.aug_100times else 'naive_svm_linear_svc', '{}shot'.format(args.shot) if args.dataset == 'Imagenet' else args.dataset.split('_')[-1])

    args.log_file = os.path.join(args.exp_dir_ft, 'results_aug_100times_{}_linear_svc.txt'.format(args.aug_100times))
    ...
    for run in range(3):
        ...
        model = LinearSVC(loss=args.loss, fit_intercept=args.fit_intercept)
