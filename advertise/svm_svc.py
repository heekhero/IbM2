...
def main():
    ...
    args.exp_dir_ft = os.path.join(args.exp_dir, 'CompareWithLinearSVM', 'SystemSearchSVC_Linear', 'aug100times' if args.aug_100times else 'naive_svm', 'C_{}'.format(args.C), '{}shot'.format(args.shot))
    ...
    for run in range(3):
        ...
        model = SVC(kernel='linear', C=args.C, decision_function_shape='ovr', random_state=0)
    ...
    write('***Final Results***            len of experiments : {}          Test_acc : {:.2f} +- {:.2f}'.format(
        len(accs), acc_mean * 100., acc_std * 100.,
    ), args.log_file)
