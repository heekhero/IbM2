...
def get_available_gpu(logic_ids = [0,1,2,3,4,5,6,7], physical_ids = [0,1,2,3,4,5,6,7]):
    os.system('nvidia-smi -i {} -q -d Memory |grep -A4 GPU|grep Free >tmp_{}_svc_system'.format(','.join([str(_id) for _id in logic_ids]), __file__.split('_')[-1]))
    memory_gpu = np.array([int(x.split()[2]) for x in open('tmp_{}_svc_system'.format(__file__.split('_')[-1]), 'r').readlines()])
