...
def get_available_gpu(logic_ids = [0,1,2,3,4,5,6,7], physical_ids = [0,1,2,3,4,5,6,7]):
    os.system('nvidia-smi -i {} -q -d Memory |grep -A4 GPU|grep Free >tmp_V100_etf'.format(','.join([str(_id) for _id in logic_ids])))
    memory_gpu = np.array([int(x.split()[2]) for x in open('tmp_V100_etf', 'r').readlines()])
        ...
        for shot in shots:
            for _t in ts:
                ...
                with open('out/extract_features_fsl_distributed/{}_{}shot_t_{}.out'.format(arch, shot, _t), 'w') as f:
                    base_list = ['python', 'advertise/extract_features_aug100times_fsl_distributed.py', '--shot', shot, '--arch', arch, '--eps', '50', '--batch_size', '128']
